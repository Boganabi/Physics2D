...
public class NoSpring : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        whatTheRopeIsConnectedTo.transform.position = anchor.transform.position;
    ...
    private void Update()
    {
        ...
        if (!movableCube.selectedObject)
        {
            ...
            interactable.transform.position = whatIsHangingFromTheRope.transform.position;
            // whatIsHangingFromTheRope.position = 
        }
        else
        {
            // move rope to object position
            allRopeSections[allRopeSections.Count - 1] = new RopeSection(whatIsHangingFromTheRope.position);

            whatIsHangingFromTheRope.transform.position = interactable.transform.position;
        }

        //set our x rotation to 0 so it doesnt become invisible in 2D space
        whatIsHangingFromTheRope.eulerAngles = new Vector3(
            0,
            0,
            whatIsHangingFromTheRope.eulerAngles.z
        );
    }
